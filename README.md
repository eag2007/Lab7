...
Клиент-серверное приложение для управления коллекцией маршрутов (`Route`). Сервер поддерживает многопоточную обработку запросов, хранение данных в PostgreSQL и авторизацию пользователей. Клиент является многопоточным приложением. В нём выделены 3 основных потока: ```main``` (считывает ввод и отправляет данные), ```printer``` (печатает ответы с сервера), ```reader``` считывает все ответы с сервера и записывает их в очередь. Реализована отправка команд на сервер, их обработка, отправка результата, также сервер умеет отправлять уведомления об изменениях в коллекции.
...
- **Шардирование 3 БД** - БД хостяться на 3 портах
